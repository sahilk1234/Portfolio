              ...
              <div className="bg-secondary border-border rounded-2xl border p-6">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <p className="text-tertiary-content text-sm">{experience.company}</p>
                  </div>
                  {experience.period ? (
                ...
                <p className="text-tertiary-content mt-4 text-sm leading-6">
                  {experience.summary}
                </p>
                {experience.highlights?.length ? (
                  <ul className="mt-4 grid gap-2 sm:grid-cols-2">
                    {experience.highlights.map((item) => (
                      <li key={item} className="text-primary-content flex items-start gap-2 text-sm">
